...
// uint32_t rrpv_LIME[8192][LLC_WAYS];
...
// SET DUELING PARAM
uint64_t red_misses;
uint64_t lime_misses;

// 0 for red, 1 for lime
bool red_or_lime;
...
uint32_t GetVictimInSet_RED (uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{
	uint8_t present;
	uint64_t PCentry;
	uint64_t block;
...
void UpdateReplacementState_RED (uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
...
// initialize replacement state
void InitReplacementState_LIME()
{
    int cfg = get_config_number();
    if(cfg==1 || cfg==2){
        NUM_CORE_LIME=1;
        LLC_SETS_LIME  = NUM_CORE_LIME*2048;
        LLC_SETS_LOG2 = 11;
        ALIAS_TAB_SIZE = NUM_CORE_LIME*64 ;
    }
    else if(cfg == 3 || cfg == 4){
        NUM_CORE_LIME=4;
        LLC_SETS_LIME  = NUM_CORE_LIME*2048;
    ...
    // for (int i=0; i<LLC_SETS_LIME; i++) {
    //     for (int j=0; j<LLC_WAYS; j++) {
    //         rrpv_LIME[i][j] = maxRRPV_LIME;
    //     }
    // }
    ...
    int sample_sets_num = 15*NUM_CORE;
    if(cfg==1 || cfg == 2){
        int random_number[15]={46, 257, 406, 510, 772, 880, 981, 1163, 1206, 1436, 1543, 1644, 1810, 1956, 2017};
...
// find replacement victim
// return value should be 0 ~ 15 or 16 (bypass)
uint32_t GetVictimInSet_LIME (uint32_t cpu, uint32_t set, const BLOCK *current_set, uint64_t PC, uint64_t paddr, uint32_t type)
{
    if(type == 3)
        return 0;
...
// called on every cache hit and cache fill
void UpdateReplacementState_LIME (uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
...
{
    InitReplacementState_RED();
    InitReplacementState_LIME();
...
{
    // if(set % 2 == 0){
    //     GetVictimInSet_RED(cpu, set, current_set, PC, paddr, type);
    // }
    // else{
    //     GetVictimInSet_LIME(cpu, set, current_set, PC, paddr, type);
    // }

    // 5 MSB
    uint32_t constituency = set >> 6;
    // 5 LSB
    uint32_t offset = set & 63;

    uint32_t offset_compl = offset ^ 63;

    // SAMPLED SETS
    if(constituency == offset){
        return GetVictimInSet_RED(cpu, set, current_set, PC, paddr, type);
    }
    else if(constituency == offset_compl){
        return GetVictimInSet_LIME(cpu, set, current_set, PC, paddr, type);
    }
...
void UpdateReplacementState(uint32_t cpu, uint32_t set, uint32_t way, uint64_t paddr, uint64_t PC, uint64_t victim_addr, uint32_t type, uint8_t hit)
{
    ...
    // if(set % 2 == 0){
    //     UpdateReplacementState_RED(cpu, set, way, paddr, PC, victim_addr, type, hit);
    // }
    // else{
    //     UpdateReplacementState_LIME(cpu, set, way, paddr, PC, victim_addr, type, hit);
    // }
    ...
    if(constituency == offset){
        ...
        if(red_or_lime == 0 && (float)red_misses/lime_misses > 1.05 and red_misses-lime_misses > 100){
            red_or_lime = 1;
        }

        return UpdateReplacementState_RED(cpu, set, way, paddr, PC, victim_addr, type, hit);
        ...
        if(red_or_lime == 1 && (float)lime_misses/red_misses > 1.05 and lime_misses-red_misses > 100){
            red_or_lime = 0;
        }

        return UpdateReplacementState_LIME(cpu, set, way, paddr, PC, victim_addr, type, hit);
...
void PrintStats()
{
    cout << "red misses: " << red_misses << endl;
    cout << "lime misses: " << lime_misses << endl;
    cout << "red to lime ratio " << static_cast<float>(red_misses) / lime_misses << endl;
}
